...
import { GET_ALERT_BY_PATH } from '@/lib/queries'
import { DrupalAlert } from '@/lib/types'
...
interface AlertByPathData {
  route: {
    entity: DrupalAlert
  } | null
}

async function getAlert(path: string): Promise<DrupalAlert | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<AlertByPathData>({
      query: GET_ALERT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/alerts/${slug.join('/')}`
  const item = await getAlert(path)

  if (!item) {
    return { title: 'Alert Not Found | Town' }
  }

  return {
    title: `${item.title} | Town`,
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).affectedArea && (
                    <div>
                      <dt className="text-sm text-gray-500">Affected Area</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).affectedArea}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
