            ...
            route(path: $path) {
              ... on RouteInternal {
                entity {
                  ...
                  ... on NodeDepartment {
                    __typename id title path body { processed } phone email location hours
                    departmentCategory { ... on TermInterface { id name } }
                    image { url alt width height }
                  }
                  ... on NodeOfficial {
                    __typename id title path body { processed } position
                    department { ... on TermInterface { id name } }
                    email phone office photo { url alt width height }
                  }
                  ... on NodeService {
                    __typename id title path body { processed }
                    department { ... on TermInterface { id name } }
                    onlineUrl eligibility fee image { url alt width height }
                  }
                  ... on NodeMeeting {
                    __typename id title path body { processed }
                    meetingDate { timestamp } endDate { timestamp } location
                    meetingType { ... on TermInterface { id name } }
                    agendaUrl image { url alt width height }
                  }
                  ... on NodeAlert {
                    __typename id title path body { processed }
                    alertLevel { ... on TermInterface { id name } }
                    effectiveDate { timestamp } expirationDate { timestamp } affectedArea
                  }
                }
              }
            }
